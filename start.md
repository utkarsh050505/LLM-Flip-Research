...
| :--- | :--- | :--- |
| **Interactive Pipeline Orchestrator** | 1-click end-to-end pipeline with cache scanning, model detection, quantization selection, and stage skip options. | [`project/run_pipeline.py`](file:///a:/LLMResearch/project/run_pipeline.py) |
| **Model Downloader & Manager** | Fast multi-threaded Hugging Face downloads to local cache with VRAM estimation and smoke-load tests. | [`project/download_model.py`](file:///a:/LLMResearch/project/download_model.py) |
| **Quantization Support** | Native 4-bit NF4 and 8-bit quantization via `bitsandbytes` to run 7B–9B models locally on modest GPUs (4GB–8GB VRAM). | [`project/modeling/hf_model.py`](file:///a:/LLMResearch/project/modeling/hf_model.py) |
| **Multi-Engine Inference** | Supports standard Hugging Face `transformers`, high-throughput `vLLM`, `sglang`, and multi-GPU `ddp`. | [`project/modeling/`](file:///a:/LLMResearch/project/modeling) |
| **Extensive Benchmark Suite** | Adapters for math, reasoning, and multimodal benchmarks (GSM8K, MATH500, AIME2025, GPQA, MathVista, AI2D, MMStar, etc.). | [`project/benchmarking/`](file:///a:/LLMResearch/project/benchmarking) |
| **Accuracy Curve & Flip Visualization** | Generates publication-ready dual-panel plots showing accuracy progression across thinking budgets and flip summary cards. | [`project/plot_accuracy_curve.py`](file:///a:/LLMResearch/project/plot_accuracy_curve.py) |
| **Automated Failure Taxonomy** | Compares last-correct prefix traces against final failed traces using LLM judges to label root causes. | [`project/taxonomy/`](file:///a:/LLMResearch/project/taxonomy) |
| **Diagnosis Inspector** | Terminal-based inspection tool to browse overthinking case studies with exact reasoning snippets. | [`project/show_diagnosis.py`](file:///a:/LLMResearch/project/show_diagnosis.py) |
...
```text
a:\LLMResearch\
├── start.md                        # This onboarding guide
